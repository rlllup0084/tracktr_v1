...
export const createVehicleSchema = z.object({
  vehicle_name: z
    .string()
    .max(100)
    .nonempty({ message: 'Vehicle Name is required.' }), // ok
  vin: z
    .string()
    .length(17, { message: 'VIN must be exactly 17 characters long' }), // ok
  ...
  make: z.string().nonempty({ message: 'Make is required' }).max(100), // ok
  model: z.string().nonempty({ message: 'Model is required' }).max(50), // ok eg. Corolla, Camry, etc.
  // year must be between 1900 and the current year plus 1
  year: z
    .number()
    .int()
    .min(1900)
    .max(new Date().getFullYear() + 1), // ok
  // Style --> optional from vin decoder
  options: z
    .string()
    .max(1000, { message: 'Options must be less than 1000 characters' }),
  colors: z.string().max(100),
  ...
  ), // --> not in the appwrite yet
  // -------- populate automatically if style is provided from vin decoder --------
  style_selected: z.string().max(30),
  style_name: z.string().max(100), // eg. LT 4dr Extended Cab 6.5 ft. SB (4.8L 8cyl 4A)
  ...
  vehicle_style: z.string().max(50), // eg. sedan, coupe, suv, etc. --> not in the appwrite yet
  ...
  rpm_horsepower: z.number().int().min(0), // eg. 6000 --> not in the appwrite yet
  rpm_torque: z.number().int().min(0), // eg. 4000 --> not in the appwrite yet
  fuel_capacity: z.string().max(50), // in gallons or liters eg. 20 gal or 20 L
  // rpm: z.number().int().min(0), // e.g. 6000 --> remove this in the appwrite
...
export const updateAcquisitionSchema = z.object({
  ...
  acquisition_warranty_expiration: z
    .string()
    .regex(/^\d{4}-\d{2}-\d{2}$/)
    .refine((date) => new Date(date) >= new Date(), {
