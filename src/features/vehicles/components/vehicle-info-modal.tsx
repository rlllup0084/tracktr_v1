...
import { type VinVehicleData, vinVehicleDataSchema } from "../utils/parseVehicleData"
...
const VehicleInfoModal = ({
  ...
  const vehicleSpecs: SpecItem[] = [
    {
      label: "Vehicle Name",
      value: `${data.years?.[0]?.year ?? ""} ${data.make?.name ?? ""} ${data.model?.name ?? ""}`,
      valueType: "input",
    ...
    { label: "Body Type", value: `${data.years?.[0]?.styles?.[0]?.submodel?.body ?? ""}`, valueType: "input" }, // Crew Cab
    { label: "Trim", value: `${data.years?.[0]?.styles?.[0]?.trim ?? ""}`, valueType: "input" }, // XFE
    ...
    {
      label: "Engine Name",
      value: `${data.engine?.name ?? ""} ${data.engine?.configuration ?? ""} ${data.engine?.cylinder ?? ""}`,
      valueType: "input",
    }, // Engine (5.3L V8)
    {
      label: "Engine Type",
  ...
  const [selectedStyle, setSelectedStyle] = useState(data.years?.[0]?.styles?.[0]?.name ?? "")
  const [selectedBodyType, setSelectedBodyType] = useState(data.years?.[0]?.styles?.[0]?.submodel?.body ?? "N/A")
  const [selectedTrim, setSelectedTrim] = useState(data.years?.[0]?.styles?.[0]?.trim ?? "N/A")

  const handleStyleChange = (styleName: string) => {
    setSelectedStyle(styleName)
    const selectedStyleData = data.years?.[0]?.styles?.find((style) => style.name === styleName)
    ...
    return (
      <div className="space-y-4 px-1">
        <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
          ...
          <TabsContent value="transmission">{renderTransmissionFields()}</TabsContent>
        </Tabs>
      </div>
    )
  }
        ...
        render={({ field }) => (
          <FormItem>
            ...
            <FormControl>
              <Input
                {...field}
                defaultValue={`${data.years?.[0]?.year ?? ""} ${data.make?.name ?? ""} ${data.model?.name ?? ""}`}
              />
            </FormControl>
          </FormItem>
        )}
      ...
      <FormField
        ...
        render={({ field }) => (
          <FormItem className="w-full">
            ...
            <FormControl>
              <Input {...field} value={data.categories?.primaryBodyType ?? ""} />
            </FormControl>
          </FormItem>
        )}
      />
      ...
      <FormField
        ...
        render={({ field }) => (
          <FormItem>
            ...
            <FormControl>
              <Input {...field} value={data.transmission?.transmissionType ?? ""} />
            </FormControl>
          </FormItem>
        )}
      />
      <FormField
        ...
        render={({ field }) => (
          <FormItem className="w-full">
            ...
            <FormControl>
              <Input {...field} value={data.transmission?.numberOfSpeeds ?? ""} />
            </FormControl>
          </FormItem>
        )}
      />
    ...
    return (
      <div className="space-y-4">
        <Tabs defaultValue="general" className="w-full">
          ...
          <TabsContent value="general">{renderReviewSection(vehicleSpecs.slice(0, 15))}</TabsContent>
          <TabsContent value="engine">{renderReviewSection(vehicleSpecs.slice(15, 30))}</TabsContent>
          <TabsContent value="transmission">{renderReviewSection(vehicleSpecs.slice(30))}</TabsContent>
        </Tabs>
      </div>
    )
  }
